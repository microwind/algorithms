...
const MAX_VERTICES: number = 100;

// 邻接矩阵表示图
let graph: number[][] = Array(MAX_VERTICES).fill(0).map(() => Array(MAX_VERTICES).fill(0));
let visited: boolean[] = Array(MAX_VERTICES).fill(false);
 ...
 * 查找所有连通分量
 */
function findConnectedComponents(): void {
