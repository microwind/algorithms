...
```
- 能清晰地理解和描述业务问题。对于程序员来讲并不是件容易的事情，很多人会做，但未必会说。
- 从表面需求挖掘真实需求。只有真正地理解了业务本质才能找到真实的需求点。
- 转化为AI能理解的指令。AI不是人，虽然我们可以把它当作人，沟通时尽量用AI能懂的方式。
```

**2. 系统设计能力**
```
- 能定义问题的边界和约束。设计就是定义流程机制，把各功能模块边界与职责定义清晰了，设计也就完成了。
- 进行容量规划和性能权衡。互联网里最看重的是数据，需要规划好系统的容量与吞吐量，这样才能保证性能。
- 设计系统的整体架构。从顶而下或从外而内，都要全局考虑，把整体框架想清楚了，具体的功能开发才不会走样。
```

**3. 算法思想能力**
```
- 能用算法思想指导AI选择方案。算法有很多种，但算法思想只有那些。遇到问题时我们都要想想用哪种思路来解决，并告诉AI。
- 理解复杂度和性能权衡。对于复杂问题能够抽象建模，并且在复杂度和性能以及成本之间找到平衡。
- 验证AI生成的代码质量。AI生成代码并不完全可靠，你需要检查AI代码的整体架构和核心功能实现。
```

**4. 指导协调能力**
```
- 能用清晰的语言指导AI。用清晰的语言表达出来，需要一定的结构化思维，把你真是想要的简洁明了地告诉AI。
- 能根据结果快速调整策略。要及时检查AI的生成结果，对于误差要及时纠偏，策略是在不断尝试中调整出来的。
- 能多轮迭代优化结果。无论是自己开发还是AI开发，都需要多轮迭代。需要把之前的共识方案保存下来，帮助AI记忆。
```

**5. 质量验证能力**

```
- 能评估AI的工作质量。AI的工作大部分没问题，但在技术选型以及技术细节会出现偏差，要有能力评估AI的产出。
- 能识别AI的弱点和局限。AI的优点很明显，那就是速度快、知识广，但也有弱点就是容易出现幻觉，知识也不够新颖。
- 能提出改进方向。工作目标还是人创造的，AI只是帮你来实现。我们要能提出具体的改进方向，告诉AI往哪里走。
```
