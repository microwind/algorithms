...
/**
 * linearSearch - 线性查找基础版本 - 查找第一个匹配项
 * 
 * 算法原理：
 * 1. 从数组开头逐个遍历元素
 * 2. 比较当前元素是否等于目标值
 ...
 * @param {Array} arr 待搜索数组
 * @param {*} target 目标值
 * @returns {Array<number>} 包含所有匹配索引的数组
 */
 ...
 * @returns {number} 找到返回索引，未找到返回-1
 */
function linearSearchGeneric(arr, target) {
    // 边界检查
    if (!arr || arr.length === 0) {
        return -1;
    }
...
function main() {
    ...
    for (let i = 0; i < strArr.length; i++) {
        if (strArr[i] === strTarget) {
            strAllIndices.push(i);
        }
    }
...
/*
打印结果
jarry@Mac linear-search % node linear_search.js
=== 线性查找演示 ===
