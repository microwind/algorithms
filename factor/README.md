...
首先什么是质因数算法？即任意一个合数可以分解为多个质数相乘。

例如：20 = 2 * 2 * 5; 30 = 2 * 3 * 5; 100 = 2 * 2 * 5 * 5。

按数学运算：列竖式或左右分解，拿能被整除的最小的素数做除数，不断整除得到新的得数，最后把所有的除数和最后的得数连起来就是。

- 则先从最小的质数2开始整除被分解数
- 当能够整除该质数时，将数字赋值为整除后的得数，得到新的分解数
- 继续从2开始整除，如果2无法整除，则递加到新的质数作为除数 
- 直到将数字分解完，也就是递增数的平方大于被分解数为止
...
5. 直到除数的平方大于要分解的值，终止循环。 最后所有可整除的除数与最后的大于1的得数就是结果。
