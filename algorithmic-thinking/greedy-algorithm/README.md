...
#### 缺点
- **适用范围限制**：只能解决具有贪心性质的问题
- **容易出错**：需要正确识别问题的贪心性质
- **证明困难**：证明贪心选择的正确性可能很复杂
- **失效风险**：对于某些问题，贪心不能得到最优解

### 操作方式
- **问题分析**：识别贪心选择的标准
- **排序或优先选择**：按某种标准对输入进行排序或优先级处理
- **迭代选择**：依次做出贪心选择
- **验证可行性**：确保选择不违反问题的约束条件
...
- **活动选择**：选择最多不重叠的活动
- **区间问题**：区间覆盖、区间调度
- **硬币兑换**：使用最少硬币（在某些币种组合下）
- **分配问题**：资源分配、任务分配
- **图论问题**：Dijkstra算法、Kruskal算法、Prim算法
- **其他问题**：贪心法求解的霍夫曼编码、跳跃游戏等
