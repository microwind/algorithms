...
class BubbleSort {
   ...
   * @param label 数组的标签说明
   */
  private printArray(arr: number[], label: string): void {
   ...
   * @param name 测试名称
   */
  private performanceTest(sortFunc: (arr: number[]) => void, arr: number[], name: string): void {
    // 创建数组副本，避免修改原数组
    const testArr = [...arr];
    this.printArray(testArr, `${name}原始数组`);
    ...
    this.printArray(testArr, `${name}排序结果`);
    console.log(''); // 空行分隔
  }
