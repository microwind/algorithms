 ...
 * 打印堆的树形结构
 */
void printHeapTree(List<int> arr, String title) {
  if (arr.length < 7) {
    print('$title: ${arr.join(', ')}');
    return;
  }
...
void performanceTest(Function(List<int>) sortFunc, List<int> arr, String name) {
  ...
  print('$name: ${(stopwatch.elapsedMilliseconds.toDouble() / 1000).toStringAsFixed(3)}ms');
 ...
 * 带步骤打印的堆化函数
 */
void heapifyWithSteps(List<int> arr, int n, int i, int parentStep) {
  int largest = i;
  int left = 2 * i + 1;
  int right = 2 * i + 2;
 ...
 * 堆化辅助函数 - 最小堆
 */
void heapifyMin(List<int> arr, int n, int i) {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
void heapSort5(List<int> arr) {
  print('heapSort5 non-recursive:');
  
  // 根据完全二叉树结构性质，父子节点与数组下标的关系
  int getParent(int i) => (i - 1) ~/ 2;
  int getLeft(int i) => 2 * i + 1;
  int getRight(int i) => 2 * i + 2;
  
  /**
   * 始终保持大顶堆特性, 构建大顶堆的非递归写法
   */
  void maxHeapify(List<int> array, int idx, int size) {
