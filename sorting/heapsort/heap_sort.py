...
def print_heap_tree(arr, title):
    """打印堆的树形结构"""
    if len(arr) < 7:
        print(f"{title}: [{', '.join(map(str, arr))}]")
        return
    
    print(title)
    print(f"              {arr[0]}(0)")
    print("          /         \\")
    print(f"        {arr[1]}(1)        {arr[2]}(2)")
    print("       /    \\      /     \\")
    print(f"   {arr[3]}(3)  {arr[4]}(4)  {arr[5]}(5)  {arr[6]}(6)")
    print()
...
def heapify_with_steps(arr, n, i, parent_step):
    """带步骤打印的堆化函数"""
    largest = i
    left = 2 * i + 1
    right = 2 * i + 2
    ...
    if largest != i:
        ...
        new_left = 2 * largest + 1
        new_right = 2 * largest + 2
        max_idx = largest
        
        if new_left < n and arr[new_left] > arr[max_idx]:
            max_idx = new_left
        if new_right < n and arr[new_right] > arr[max_idx]:
            max_idx = new_right
        
        sub_step = '1'
        print(f"{parent_step}.{sub_step} maxHeapify: idx= {largest} left= {new_left} right= {new_right} max= {max_idx} size= {n}")
        print_heap_tree(arr, "")
...
def heap_sort1(arr):
    ...
    for i in range(n // 2 - 1, -1, -1):
        ...
        print(f"{step_count}. maxHeapify: idx= {i} left= {left} right= {right} max= {max_idx} size= {n}")
        step_count += 1
    ...
    for i in range(n - 1, 0, -1):
        print(f"{step_count}. swap, child = {i}:")
        # 将堆顶元素（最大）与末尾元素交换
...
def heap_sort5(arr):
    ...
    def max_heapify(arr, idx, size):
        """始终保持大顶堆特性, 构建大顶堆的非递归写法"""
        current = arr[idx]
        child = get_left(idx)
        # 从当前位置的左节点开始遍历
        while child < size:
            print(f'current= {current}  idx= {idx}  child= {child}  size= {size}')
    ...
    while parent >= 0:
        ...
        print(f'parent sort: {parent} {arr}')
        parent -= 1
    
    print(f'child start: parent={parent}  child={child}')
...
"""
jarry@Mac heapsort % python heap_sort.py 
...
排序后数组: [7, 8, 9, 10, 11, 12, 13]
heapSort5 non-recursive: 0.028ms
