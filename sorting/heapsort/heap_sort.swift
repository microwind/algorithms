...
func printArray(_ arr: [Int], _ label: String) {
    print("\(label): \(arr)")
}

// printHeapTree 打印堆的树形结构
func printHeapTree(_ arr: [Int], _ title: String) {
    if arr.count < 7 {
        print("\(title): \(arr)")
        return
    }
    ...
    print("       /    \\      /     \\")
    print("   \(arr[3])(3)  \(arr[4])(4)  \(arr[5])(5)  \(arr[6])(6)")
    print()
}
 ...
 * 性能测试辅助函数
 */
func performanceTest(_ sortFunc: (inout [Int]) -> Void, _ arr: [Int], _ name: String) {
    // 创建数组副本，避免修改原数组
    var testArr = arr
    ...
    print(String(format: "%@: %.3fms", name, duration))
 ...
 * 带步骤打印的堆化函数
 */
func heapifyWithSteps(_ arr: inout [Int], _ n: Int, _ i: Int, _ parentStep: Int) {
    var largest = i
    let left = 2 * i + 1
    let right = 2 * i + 2
    ...
    if largest != i {
        ...
        let newLeft = 2 * largest + 1
        let newRight = 2 * largest + 2
        var maxIdx = largest
        
        if newLeft < n && arr[newLeft] > arr[maxIdx] {
            maxIdx = newLeft
        }
        if newRight < n && arr[newRight] > arr[maxIdx] {
            maxIdx = newRight
        }
        
        print("\(parentStep).1 maxHeapify: idx= \(largest) left= \(newLeft) right= \(newRight) max= \(maxIdx) size= \(n)")
        printHeapTree(arr, "")
 ...
 */
func heapSort1(_ arr: inout [Int]) {
    print("heapSort1 max-heap:")
    let n = arr.count
    ...
    for i in (0..<n/2).reversed() {
        ...
        print("\(stepCount). maxHeapify: idx= \(i) left= \(left) right= \(right) max= \(maxIdx) size= \(n)")
        stepCount += 1
 ...
 * 堆化辅助函数 - 最小堆
 */
func heapifyMin(_ arr: inout [Int], _ n: Int, _ i: Int) {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
func heapSort2(_ arr: inout [Int]) {
    print("heapSort2 min-heap:")
    let n = arr.count
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
func heapSort3(_ arr: inout [Int]) {
    print("heapSort3 iterative:")
    let n = arr.count
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
func heapSort4(_ arr: inout [Int]) {
    print("heapSort4 bottom-up:")
    let n = arr.count
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
func heapSort5(_ arr: inout [Int]) {
    print("heapSort5 non-recursive:")
    
    // 根据完全二叉树结构性质，父子节点与数组下标的关系
    let getParent = { (i: Int) -> Int in (i - 1) / 2 }
    let getLeft = { (i: Int) -> Int in 2 * i + 1 }
    // let getRight = { (i: Int) -> Int in 2 * i + 2 }
    
    /**
     * 始终保持大顶堆特性, 构建大顶堆的非递归写法
     */
    func maxHeapify(_ array: inout [Int], _ idx: Int, _ size: Int) {
        let current = array[idx]
        var idxMut = idx
        var child = getLeft(idxMut)
        // 从当前位置的左节点开始遍历
        while child < size {
            print("current= \(current)  idx= \(idxMut)  child= \(child)  size= \(size)")
    ...
    while parent >= 0 {
        ...
        print("parent sort: \(parent)", terminator: " ")
        printArray(arr, "")
        parent -= 1
    }
    
    print("child start: parent=\(parent)  child=\(child)")
    ...
    while child > 0 {
        ...
        print("child sort: \(child)", terminator: " ")
        printArray(arr, "")
        child -= 1
    }
...
print("=== 算法对比总结 ===")
print("1. heapSort1 max-heap：经典实现，升序排序")
print("2. heapSort2 min-heap：反向思维，直观易懂")
print("3. heapSort3 iterative：避免递归，性能优化")
print("4. heapSort4 bottom-up：高效构建，减少比较")
print("5. heapSort5 non-recursive：非递归堆化，父子节点计算函数")

/* 打印结果
jarry@Mac heapsort % swift heap_sort.swift
...
排序后数组: [7, 8, 9, 10, 11, 12, 13]
heapSort5 non-recursive: 0.044ms
