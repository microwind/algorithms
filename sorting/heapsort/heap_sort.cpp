 ...
 * 打印堆的树形结构
 */
void printHeapTree(const vector<int>& arr, const string& title) {
    if (arr.size() < 7) {
        cout << title << ": [";
        for (size_t i = 0; i < arr.size(); i++) {
            cout << arr[i];
    ...
    cout << "              " << arr[0] << "(0)" << endl;
    cout << "          /         \\" << endl;
    cout << "        " << arr[1] << "(1)        " << arr[2] << "(2)" << endl;
    cout << "       /    \\      /     \\" << endl;
    cout << "   " << arr[3] << "(3)  " << arr[4] << "(4)  " << arr[5] << "(5)  " << arr[6] << "(6)" << endl;
    cout << endl;
}
...
// 测试数据：
const vector<int> testData = {7, 11, 9, 10, 12, 13, 8};

/**
 * 带步骤打印的堆化函数
 */
void heapifyWithSteps(vector<int>& arr, int n, int i, int parentStep) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
    ...
    if (largest != i) {
        ...
        cout << parentStep << ".1 maxHeapify: idx= " << largest << " left= " << newLeft << " right= " << newRight << " max= " << maxIdx << " size= " << n << endl;
        printHeapTree(arr, "");
...
void heapSort1(vector<int>& arr) {
    ...
    for (int i = n / 2 - 1; i >= 0; i--) {
        ...
        cout << stepCount << ". maxHeapify: idx= " << i << " left= " << left << " right= " << right << " max= " << maxIdx << " size= " << n << endl;
        stepCount++;
    ...
    cout << "子节点堆排序:" << endl;
    for (int i = n - 1; i > 0; i--) {
        cout << stepCount << ". swap, child = " << i << ":" << endl;
        // 将堆顶元素（最大）与末尾元素交换
        int temp = arr[0];
        arr[0] = arr[i];
        arr[i] = temp;
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
void heapSort3(vector<int>& arr) {
    cout << "heapSort3 iterative:" << endl;
    int n = arr.size();
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
void heapSort4(vector<int>& arr) {
    cout << "heapSort4 bottom-up:" << endl;
    int n = arr.size();
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
void heapSort5(vector<int>& arr) {
    cout << "heapSort5 non-recursive:" << endl;
    
    // 根据完全二叉树结构性质，父子节点与数组下标的关系
    auto getParent = [](int i) { return (i - 1) / 2; };
    auto getLeft = [](int i) { return 2 * i + 1; };
    auto getRight = [](int i) { return 2 * i + 2; };
    
    /**
     * 始终保持大顶堆特性, 构建大顶堆的非递归写法
     */
    auto maxHeapify = [&](vector<int>& array, int idx, int size) {
        int current = array[idx];
        int child = getLeft(idx);
        // 从当前位置的左节点开始遍历
        for (; child < size;) {
            cout << "current= " << current << "  idx= " << idx << "  child= " << child << "  size= " << size << endl;
    ...
    while (parent >= 0) {
        ...
        cout << "parent sort: " << parent << " ";
        printArray(arr, "");
        parent--;
    }
    
    cout << "child start: parent=" << parent << "  child=" << child << endl;
...
int main() {
    ...
    cout << "=== 算法对比总结 ===" << endl;
    cout << "1. heapSort1 max-heap：经典实现，升序排序" << endl;
    cout << "2. heapSort2 min-heap：反向思维，直观易懂" << endl;
    cout << "3. heapSort3 iterative：避免递归，性能优化" << endl;
    cout << "4. heapSort4 bottom-up：高效构建，减少比较" << endl;
    cout << "5. heapSort5 non-recursive：非递归堆化，父子节点计算函数" << endl;
    
    return 0;
}
/*
打印结果
jarry@Mac heapsort % g++ heap_sort.cpp -o heap_sort && ./heap_sort
...
排序后数组: [7, 8, 9, 10, 11, 12, 13]
heapSort5 non-recursive: 0.062ms
