 ...
 * 带步骤打印的堆化函数
 */
func heapifyWithSteps(arr []int, n, i, parentStep int) {
	largest := i
	left := 2*i + 1
	right := 2*i + 2
	...
	if largest != i {
		...
		fmt.Printf("%d.1 maxHeapify: idx= %d left= %d right= %d max= %d size= %d\n",
			parentStep, largest, newLeft, newRight, maxIdx, n)
		printHeapTree(arr, "")
...
func heapSort1(arr []int) {
	fmt.Println("heapSort1 max-heap:")
	n := len(arr)
	...
	for i := n/2 - 1; i >= 0; i-- {
		...
		fmt.Printf("%d. maxHeapify: idx= %d left= %d right= %d max= %d size= %d\n",
...
func heapSort2(arr []int) {
	fmt.Println("heapSort2 min-heap:")
	n := len(arr)
...
func heapSort3(arr []int) {
	fmt.Println("heapSort3 iterative:")
	n := len(arr)
...
func heapSort4(arr []int) {
	fmt.Println("heapSort4 bottom-up:")
	n := len(arr)
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
func heapSort5(arr []int) {
	fmt.Println("heapSort5 non-recursive:")

	// 根据完全二叉树结构性质，父子节点与数组下标的关系
	getParent := func(i int) int { return (i - 1) / 2 }
	getLeft := func(i int) int { return 2*i + 1 }
	// getRight := func(i int) int { return 2*i + 2 }
	...
	maxHeapify := func(array []int, idx, size int) {
		...
		// 从当前位置的左节点开始遍历
		for child < size {
			fmt.Printf("current= %v  idx= %v  child= %v  size= %v\n", current, idx, child, size)
	...
	for parent >= 0 {
		...
		fmt.Printf("parent sort: %v ", parent)
		printArray(arr, "")
		parent--
	}

	fmt.Printf("child start: parent=%v  child=%v\n", parent, child)
...
func main() {
	...
	fmt.Println("=== 算法对比总结 ===")
	fmt.Println("1. heapSort1 max-heap：经典实现，升序排序")
	fmt.Println("2. heapSort2 min-heap：反向思维，直观易懂")
	fmt.Println("3. heapSort3 iterative：避免递归，性能优化")
	fmt.Println("4. heapSort4 bottom-up：高效构建，减少比较")
	fmt.Println("5. heapSort5 non-recursive：非递归堆化，父子节点计算函数")
}

/*
打印结果
jarry@Mac heapsort % go run heap_sort.go
...
排序后数组: [[7 8 9 10 11 12 13]]
heapSort5 non-recursive: 0.042ms
