 ...
 * 打印堆的树形结构
 */
function printHeapTree(arr: number[], title: string): void {
    if (arr.length < 7) {
 ...
 * 带步骤打印的堆化函数
 */
function heapifyWithSteps(arr: number[], n: number, i: number, parentStep: number): void {
    let largest = i;
    const left = 2 * i + 1;
    const right = 2 * i + 2;
    ...
    if (largest !== i) {
        ...
        const newLeft = 2 * largest + 1;
        const newRight = 2 * largest + 2;
        let maxIdx = largest;
        
        if (newLeft < n && arr[newLeft] > arr[maxIdx]) maxIdx = newLeft;
        if (newRight < n && arr[newRight] > arr[maxIdx]) maxIdx = newRight;
        
        console.log(`${parentStep}.1 maxHeapify: idx= ${largest} left= ${newLeft} right= ${newRight} max= ${maxIdx} size= ${n}`);
        printHeapTree(arr, '');
 ...
 */
function heapSort1(arr: number[]): void {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
function heapSort2(arr: number[]): void {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
function heapSort3(arr: number[]): void {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
function heapSort4(arr: number[]): void {
 ...
 * 稳定性：不稳定 - 堆化过程可能改变相等元素的相对位置
 */
function heapSort5(arr: number[]): void {
     ...
     */
    const maxHeapify = (array: number[], idx: number, size: number) => {
        let current = array[idx];
        let idxMut = idx;
        let child = getLeft(idxMut);
        // 从当前位置的左节点开始遍历
        while (child < size) {
            console.log(`current= ${current}  idx= ${idxMut}  child= ${child}  size= ${size}`);
    ...
    console.log(`child start: parent=${parent}  child=${child}`);
...
/* 打印结果
jarry@Mac heapsort % ts-node HeapSort.ts 
...
: [7, 8, 9, 10, 11, 12, 13]
排序后数组: [7, 8, 9, 10, 11, 12, 13]
heapSort5 non-recursive: 0.494ms
