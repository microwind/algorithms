 ...
 */
function selectionSort1(arr: number[]): void {
    console.log('selectionSort1 basic:');
    const n: number = arr.length;
    
    // 外循环：控制排序轮数，每轮确定一个最小值的位置
    for (let i = 0; i < n - 1; i++) {
        let minIndex: number = i; // 记录最小元素的索引
 ...
 * 稳定性：不稳定 - 交换可能改变相等元素的相对位置
 */
function selectionSort2(arr: number[]): void {
    console.log('selectionSort2 descending:');
    const n: number = arr.length;
    
    // 外循环：控制排序轮数，每轮确定一个最大值的位置
    for (let i = 0; i < n - 1; i++) {
        let maxIndex: number = i; // 记录最大元素的索引
