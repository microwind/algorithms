...
f([7]) f([])
  [7]
*/
List<int> quickSort1(List<int> arr) {
  // 第一步：递归终止条件
  ...
  List<int> result = [];
  result.addAll(quickSort1(left));
  result.add(pivot);
  result.addAll(quickSort1(right));
...
int partition(List<int> arr, int left, int right) {
  ...
  print('partitioned arr= $arr partitionIndex: $partitionIndex '
        'left= ${arr.sublist(left, partitionIndex)} arr[partitionIndex]= ${arr[partitionIndex]} '
        'right= ${arr.sublist(partitionIndex, right + 1)} $arr');
 ...
 * @returns 排序后的数组
 */
List<int> quickSort2(List<int> arr, [int left = 0, int? right]) {
  // 第一步：初始化参数
 ...
 * @returns 排序后的数组
 */
List<int> quickSort3(List<int> arr, [int left = 0, int? right]) {
  // 第一步：初始化指针和基准
  right = right ?? arr.length - 1;
  int i = left;
  int j = right;
  // 关键点：确定中间位置，基于中间位置不停左右交换
  int midIndex = ((i + j) / 2).floor();
  int pivot = arr[midIndex];
  ...
  while (i <= j) {
    ...
    while (arr[i] < pivot) {
      print('arr[i] < pivot: i=$i j=$j pivot=$pivot');
      i++;
    }
    // 步骤2.2：右指针扫描，找到小于基准的元素
    // 关键点：当前右侧大于基准时左移，直到找出比基准值小的位置来
    while (arr[j] > pivot) {
      print('arr[j] > pivot: i=$i j=$j pivot=$pivot');
      j--;
    }

    print('  left=$left right=$right i=$i j=$j midIndex=$midIndex pivot=$pivot arr[]=$arr');
  ...
  if (left < j) {
    print('left < j:recursion:  left=$left right=$right i=$i j=$j arr[]$arr');
    quickSort3(arr, left, j);
  }
  // 步骤3.2：递归处理右子数组
  // 关键点：基数位置小于右侧，不断递归右侧部分
  if (i < right) {
    print('i < right:recursion:  left=$left right=$right i=$i j=$j arr[]$arr');
 ...
 * @returns 排序后的数组
 */
List<int> quickSort4(List<int> arr, [int left = 0, int? right]) {
  // 第一步：初始化参数
  // 关键点：设置默认值，确保函数可以单独调用
  right = right ?? arr.length - 1;
 ...
 * @returns 基准元素的最终位置
 */
int partitionLomuto(List<int> arr, int low, int high) {
  // 第一步：初始化基准和指针
 ...
 * @returns 排序后的数组
 */
List<int> quickSort5(List<int> arr, [int left = 0, int? right]) {
  // 第一步：递归终止条件检查
  right = right ?? arr.length - 1;
  if (left >= right) {
    return arr;
 ...
 * @returns 排序后的数组
 */
List<int> quickSort6(List<int> arr, [int left = 0, int? right]) {
  // 第一步：递归终止条件检查
  right = right ?? arr.length - 1;
  if (left >= right) {
    return arr;
 ...
 * @returns 排序后的数组
 */
List<int> quickSort7(List<int> arr, [int left = 0, int? right]) {
  // 第一步：递归终止条件检查
  right = right ?? arr.length - 1;
  if (left >= right) {
    return arr;
...
void main() {
  ...
  // 测试3：双向扫描递归版本
  performanceTest((arr) => quickSort3(arr, 0, arr.length - 1), testData, '双向扫描递归版本');

  // 测试4：非递归版本
  performanceTest((arr) => quickSort4(arr, 0, arr.length - 1), testData, '非递归版本');
  ...
  print('2. 标准递归版本：右侧基准，原地交换，Lomuto分区');
  print('3. 双向扫描递归版本：中间基准，双向扫描，原地交换');
  print('4. 非递归版本：栈模拟，避免递归，双向扫描');
  print('5. Lomuto分区递归版本：最后基准，单向扫描');
  print('6. Hoare分区递归版本：第一个基准，双向扫描');
  print('7. 三路分区递归版本：第一个基准，三路分区');
}
/*打印结果
jarry@Mac quicksort % dart quick_sort.dart 
