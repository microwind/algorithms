    ...
    while (gap > 0) {
        ...
        for (i in gap until n) {
            ...
            while (j >= gap && arr[j - gap] > temp) {
                println("gap=$gap i=$i j-gap=${j - gap} j=$j arr:${arr.contentToString()}")
                arr[j] = arr[j - gap]
                j -= gap
            }
    ...
    while (gap > 0) {
        ...
        for (i in gap until n) {
            ...
            while (j >= 0 && arr[j] > temp) {
                println("gap=$gap i=$i j=$j j+gap=${j + gap} arr:${arr.contentToString()}")
                arr[j + gap] = arr[j]
                j -= gap
            }
            arr[j + gap] = temp
 ...
 * @param gap 当前增量
 */
fun shellSort5(arr: IntArray, gap: Int) {
    // 递归终止条件
    if (gap <= 0) {
        return
    }
...
fun main() {
    ...
    // 测试5：递归版本（尾递归）
    performanceTest({ arr -> shellSort5(arr, arr.size / 2) }, testData, "递归版本")
...
/*
打印结果
jarry@Mac shellsort % kotlinc ShellSort.kt -include-runtime -d ShellSort.jar && java -jar ShellSort.jar
